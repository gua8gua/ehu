...
- **构建**：默认使用 **build-scripts** 下的编译脚本。在项目根目录执行：  
  **配置** `.\build-scripts\gen_compile_commands.ps1` → **编译** `.\build-scripts\build_ninja.ps1`
...
- **根目录托管脚本**：[Scripting/README.md](Scripting/README.md) — 说明 `Scripting/`（C#）与 `Ehu/src/Ehu/Scripting/`（C++）的区别、编译与部署 DLL
